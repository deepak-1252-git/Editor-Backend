...
@app.route('/word_editor', methods=['POST'])
def word_editor():
    try:
        ...
        if mode == "template":
            ...
            doc = Document()
            doc.add_heading('Official Document', 0)
            doc.add_paragraph(f"To: {name}")
            doc.add_paragraph(f"Subject: {subject}")
            doc.add_paragraph("\nThis document was automatically generated by ToolNovax Word Engine.")
            
            out_name = f"template_{uuid.uuid4().hex}.docx"
            out_path = os.path.join(OUTPUT_FOLDER, out_name)
            doc.save(out_path)
            return jsonify({"name": out_name})

        if not files:
            return jsonify({"error": "No files uploaded"}), 400

        # --- Advanced Editing / Find & Replace ---
        if mode == "edit":
            find_txt = request.form.get('find', '')
            replace_txt = request.form.get('replace', '')
            f = files[0]
            temp_path = os.path.join(UPLOAD_FOLDER, secure_filename(f.filename))
            f.save(temp_path)
            ...
            for p in doc.paragraphs:
                if find_txt in p.text:
                    p.text = p.text.replace(find_txt, replace_txt)
            
            out_name = f"edited_{uuid.uuid4().hex}.docx"
            doc.save(os.path.join(OUTPUT_FOLDER, out_name))
            os.remove(temp_path)
            return jsonify({"name": out_name})
        ...
        if mode == "merge":
            ...
            master_doc.save(os.path.join(OUTPUT_FOLDER, out_name))
            return jsonify({"name": out_name})
